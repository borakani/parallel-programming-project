...
Image *read_pgm(const char *filename) {
    ...
    // parse width and height from current line
    int w, h;
    sscanf(line, "%d %d", &w, &h);
...
void gaussian_blur(const Image *src, Image *dst) {
    ...
    for (int y = 0; y < H; y++) {
        for (int x = 0; x < W; x++) {
            float acc = 0.0f;
            // We scan the neighbours of the current pixel in the the distance of 2 for each direction  
            ...
            // We calculate the weighted sum of 25 neighbours then normalize it by dividing it by kernel sum
...
void sobel_edge(const Image *src, Image *dst) {
    int W = src->width, H = src->height;
    for (int y = 0; y < H; y++) {
        for (int x = 0; x < W; x++) {
...
int main(int argc, char *argv[]) {
    if(argc < 2) {
        fprintf(stderr, "Usafe: %s <image.pgm>\n", argv[0]);
        return 1;
    }
